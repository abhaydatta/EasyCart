...
class HeaderGridView : GridView {
    ...
    private val mHeaderViewInfos =
        ArrayList<FixedViewInfo>()

     fun initHeaderGridView() {
    ...
    private fun removeFixedViewInfo(
        v: View,
        where: ArrayList<FixedViewInfo>
    ) {
        val len = where.size
    ...
    override fun setAdapter(adapter: ListAdapter) {
        if (mHeaderViewInfos.size > 0) {
            val hadapter =
                HeaderViewGridAdapter(mHeaderViewInfos, adapter)
